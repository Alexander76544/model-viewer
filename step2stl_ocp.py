...
def main():
    ...
    try:
        ...
        # разные версии OCP по-разному раскладывают этот класс по подмодулям
        TDF_LabelSequence = None
        for mod_name, attr_name in [
            ("OCP.TDF.TDF_LabelSequence", "TDF_LabelSequence"),
            ("OCP.TColStd", "TColStd_ListOfLabel"),
        ]:
            try:
                import importlib
                mod = importlib.import_module(mod_name)
                TDF_LabelSequence = getattr(mod, attr_name)
                break
            except Exception:
                continue
        if TDF_LabelSequence is None:
            print("Не нашёл класс TDF_LabelSequence автоматически.")
            print("Выполни: python -c \"import OCP.TDF as m; print([n for n in dir(m) if 'Label' in n])\"")
            print("и пришли результат — поправим импорт под твою версию OCP.")
            sys.exit(1)
